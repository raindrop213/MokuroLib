...
class ImageProcessorApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        # 创建提示标签
        self.instruction_label = tk.Label(self, text="拖放图片生成同大小的纯白图片用于凑页")
        ...
        # 创建提示文本区域，指示用户图像已被处理，并启用换行
        self.processed_label = tk.Label(self, text="", wraplength=600, justify="center")
    ...
    def create_white_image(self):
        if self.image_path:
            ...
            white_image_path = os.path.join(save_directory, f"{name}_white{ext}")

            white_image.save(white_image_path)

            # 显示图像处理结果的提示
            self.processed_label.config(text=f"图像已成功处理并保存为:\n{white_image_path}")
