...
class ImageSplitterApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.split_button = tk.Button(self, text="从定位线分割", command=self.split_image)
        self.split_button.pack(side=tk.BOTTOM, fill=tk.X)

        self.middle_split_button = tk.Button(self, text="从正中间分割", command=self.split_image_in_middle)
        self.middle_split_button.pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def split_image(self):
        if self.image_path:
            ...
            left_image_path = os.path.join(save_directory, f"{name}_b{ext}")
            right_image_path = os.path.join(save_directory, f"{name}_a{ext}")
    ...
    def split_image_in_middle(self):
        if self.image_path:
